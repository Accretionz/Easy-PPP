...
export function getProductForBanner({id, countryCode, url}: {id: string, countryCode: string, url: string}) {
...
export async function updateCountryDiscounts(
    ...
    if (insertGroup.length > 0) {
        statements
            .push(db.insert(CountryGroupDiscountTable).values(insertGroup)
            .onConflictDoUpdate({
                target: [
